...
function ProgramStatusBoard() {
  const { data, isLoading } = useSWR("/api/programs", fetcher, { refreshInterval: 60000 });
  const programs = data?.programs ?? [];
  const updatedAt = data?.updatedAt ? new Date(data.updatedAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }) : null;

  const statusConfig: Record<string, { bg: string; text: string; dot: string; label: string }> = {
    live:     { bg: "bg-[#00C805]/10", text: "text-[#00C805]", dot: "#00C805", label: "Live" },
    building: { bg: "bg-[#00BFFF]/10", text: "text-[#00BFFF]", dot: "#00BFFF", label: "Building" },
    paused:   { bg: "bg-[#FFB800]/10", text: "text-[#FFB800]", dot: "#FFB800", label: "Paused" },
    pending:  { bg: "bg-[#9CA3AF]/10", text: "text-[#9CA3AF]", dot: "#9CA3AF", label: "Pending" },
  };
  ...
  return (
    <div className="bg-[#111111] border border-[#2A2A2A] rounded-2xl p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-xl font-bold text-white">Program Status</h2>
          <p className="text-xs text-[#9CA3AF] mt-0.5">Updated daily at 7:30 AM by Artemis</p>
        </div>
        {updatedAt && <span className="text-xs text-[#9CA3AF]">Last sync: {updatedAt}</span>}
      </div>

      {isLoading ? (
        <div className="text-center text-[#9CA3AF] py-8">Loading program status...</div>
      ) : (
        <div className="space-y-3">
          {programs.map((prog: { name: string; status: string; url?: string; revenue7d: number; revenueMonth: number; lastActivity: string; nextAction: string; updatedAt: string }) => {
            const cfg = statusConfig[prog.status] ?? statusConfig.pending;
            const hasRevenue = prog.revenue7d > 0 || prog.revenueMonth > 0;
            return (
              <div key={prog.name} className={`rounded-xl p-4 border ${cfg.bg} border-[#2A2A2A]`}>
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {prog.url ? (
                        <a href={prog.url} target="_blank" rel="noreferrer"
                          className="text-sm font-bold text-white hover:text-[#00BFFF] transition-colors truncate">
                          {prog.name} ↗
                        </a>
                      ) : (
                        <span className="text-sm font-bold text-white">{prog.name}</span>
                      )}
                      <span className={`text-xs px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${cfg.bg} ${cfg.text}`}>
                        {cfg.label}
                      </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
                      <div>
                        <p className="text-xs text-[#9CA3AF] font-medium">Yesterday / Last activity</p>
                        <p className="text-xs text-white mt-0.5">{prog.lastActivity || "—"}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#9CA3AF] font-medium">
                          {prog.status === "live" && !hasRevenue ? "Marketing / Next lever" : "Next action"}
                        </p>
                        <p className="text-xs text-white mt-0.5">{prog.nextAction || "—"}</p>
                      </div>
                    </div>
                  </div>
                  {hasRevenue && (
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs text-[#9CA3AF]">7-day</p>
                      <p className="text-lg font-extrabold text-[#00C805]">${prog.revenue7d.toLocaleString("en-US", { minimumFractionDigits: 2 })}</p>
                      <p className="text-xs text-[#9CA3AF] mt-1">Month: ${prog.revenueMonth.toLocaleString("en-US", { minimumFractionDigits: 2 })}</p>
                    </div>
                  )}
                </div>
              </div>
